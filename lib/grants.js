...
function buildPreferenceContext(grants) {
  ...
  if (!scoredGrants.length) {
    return 'No human scoring feedback yet. Score honestly per the SWB calibration scenarios; do not overfit.';
  }
  ...
  const sections = [];
  if (liked.length) sections.push(`Strong fits Jesse confirmed — find more like these: ${liked.join(' | ')}`);
  if (disliked.length) sections.push(`Poor fits Jesse rejected — avoid similar programs: ${disliked.join(' | ')}`);
