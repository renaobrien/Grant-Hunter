...
function buildSubject(grants, weekOf) {
  ...
  if (!grants.length) return `[SWB Grants] ${weekOf} — no new opportunities`;
  if (pursueCount > 0) return `[SWB Grants] ${weekOf} — ${pursueCount} pursue, ${grants.length} total`;
  return `[SWB Grants] ${weekOf} — ${grants.length} new opportunities`;
}
...
function buildEmailBody({ grants, summary, weekOf, stats, sheetUrl }) {
  ...
  const html = `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:680px;margin:0 auto;padding:24px;background:#f9fafb;">
      <div style="margin-bottom:24px;">
        <div style="font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Speech Without Borders</div>
  ...
  const text = [
    `Speech Without Borders — Weekly Grants Digest`,
    `Week of ${weekOf}`,
    '',
    summary || '',
    '',
